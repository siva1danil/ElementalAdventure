...
public class LoadWorldRequestPacketHandler : PacketRegistry.IPacketHandler {
    ...
    public void Handle(PacketConnection connection, IPacket ipacket) {
        ...
        Generator.LayoutRoom[,] layout = Generator.GenerateLayout(profile.Value.Seed, 0, new Dictionary<RoomType, int> { { RoomType.Entrance, 1 }, { RoomType.Exit, 1 }, { RoomType.Normal, 1 + (int)Math.Pow(profile.Value.Floor, 1.5) } });
        ...
        _ = connection.SendAsync(new LoadWorldResponsePacket() { Tilemap = data, Midground = type.MidgroundLayer, Walls = wallsData, PlayerPosition = (startX, startY), Exit = (exitX, exitY), Floor = profile.Value.Floor });

        (float X, float Y, AssetID Type)[] enemies = Generator.GetEnemies(layout, tilemask, type, profile.Value.Floor);
