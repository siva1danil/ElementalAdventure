...
public class NextLevelPacketHandler : PacketRegistry.IPacketHandler {
    private readonly IDatabase _database;

    public NextLevelPacketHandler(IDatabase database) {
        _database = database;
    }

    public void Handle(PacketConnection connection, IPacket ipacket) {
        if (ipacket is not NextLevelPacket packet) {
            Logger.Info("Expected NextLevelRequestPacket, got " + ipacket.GetType().Name);
            return;
        }
        ...
        profile = new PlayerProfile(profile.Value.Uid, new Random().Next(), profile.Value.Floor + 1);
